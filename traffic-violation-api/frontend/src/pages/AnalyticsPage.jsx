import { useState, useEffect } from "react";
import { useTheme }            from "../context/ThemeContext";
import { useReports }          from "../context/ReportsContext";
import SpotlightCard           from "../components/ui/SpotlightCard";
import GlitchText              from "../components/ui/GlitchText";
import AnimCounter             from "../components/ui/AnimCounter";
import Chip                    from "../components/ui/Chip";
import DonutChart              from "../components/analytics/DonutChart";
import BarChart                from "../components/analytics/BarChart";
import { ANALYTICS_DATA }      from "../data/constants";
import api                     from "../axiosConfig";

export default function AnalyticsPage() {
  const { c }       = useTheme();
  const { reports } = useReports();

  const [stats, setStats] = useState({
    totalFine:  0,
    avgFine:    0,
    thisMonth:  0,
    fineIssued: 0,
  });

  const [violationBreakdown, setViolationBreakdown] = useState([]);
  const [topZones,           setTopZones]           = useState([]);

  // ✅ Real data se stats + breakdown + zones calculate karo
  useEffect(() => {
    if (reports.length === 0) return;

    // Fine stats
    const allFines  = reports.filter(r => r.fine != null).map(r => r.fine);
    const totalFine = allFines.reduce((s, f) => s + f, 0);
    const avgFine   = allFines.length > 0 ? Math.round(totalFine / allFines.length) : 0;

    // This month count
    const now       = new Date();
    const thisMonth = reports.filter(r => {
      if (!r.time || r.time === "N/A" || r.time === "Just now") return false;
      const d = new Date(r.time);
      return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear();
    }).length;

    // Fine issued count
    const fineIssued = reports.filter(r => r.status === "FINE ISSUED").length;

    setStats({ totalFine, avgFine, thisMonth, fineIssued });

    // ✅ Violation type breakdown
    const typeCounts = {};
    reports.forEach(r => {
      const t = r.type || "Unknown";
      typeCounts[t] = (typeCounts[t] || 0) + 1;
    });
    const total  = reports.length;
    const colors = [c.red, c.accent, c.yellow, c.purple, c.green];
    const sortedTypes = Object.entries(typeCounts)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5);
    const breakdown = sortedTypes.map(([label, count], i) => ({
      label,
      val:   Math.round((count / total) * 100),
      color: colors[i] || c.muted,
    }));
    setViolationBreakdown(breakdown);

    // ✅ Location zones — real data se
    const locationCounts = {};
    reports.forEach(r => {
      const loc = r.location && r.location !== "N/A" ? r.location : null;
      if (loc) locationCounts[loc] = (locationCounts[loc] || 0) + 1;
    });
    const sortedZones = Object.entries(locationCounts)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
      .map(([name, count]) => ({ name, count }));
    setTopZones(sortedZones);

  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [reports]);

  // ✅ Backend se total fine fetch karo
  useEffect(() => {
    api.get("/violations/total-fine")
      .then(res => {
        setStats(prev => ({
          ...prev,
          totalFine: res.data.totalFine || 0,
        }));
      })
      .catch(err => console.error("Total fine fetch failed:", err));
  }, []);

  const kpis = [
    { l: "Total Reports", v: reports.length,  i: "📁", col: "accent"  },
    { l: "This Month",    v: stats.thisMonth,  i: "📅", col: "green"   },
    { l: "Fines Issued",  v: stats.fineIssued, i: "💰", col: "purple"  },
    { l: "Avg Fine (₹)",  v: stats.avgFine,    i: "📊", col: "yellow"  },
  ];

  const violations = violationBreakdown.length > 0
    ? violationBreakdown
    : [
        { label: "Red Light",  val: 38, color: c.red    },
        { label: "No Helmet",  val: 27, color: c.accent  },
        { label: "Speeding",   val: 19, color: c.yellow  },
        { label: "Wrong Side", val: 10, color: c.purple  },
        { label: "Others",     val:  6, color: c.green   },
      ];

  const maxZoneCount = topZones[0]?.count || 1;

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>

      {/* Header */}
      <div className="fade-up">
        <GlitchText text="Analytics Dashboard" color={c.text} size={21} />
        <div style={{ color: c.muted, fontSize: 13, marginTop: 5 }}>
          City-wide violation intelligence · Real-time insights
        </div>
      </div>

      {/* KPI Cards */}
      <div className="stats4" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14 }}>
        {kpis.map((s, i) => (
          <SpotlightCard key={i} style={{ padding: 18 }}>
            <div style={{ fontSize: 24, marginBottom: 6 }}>{s.i}</div>
            <AnimCounter value={s.v} color={c[s.col]} size={22} />
            <div style={{ fontSize: 11, color: c.muted, marginTop: 4 }}>{s.l}</div>
          </SpotlightCard>
        ))}
      </div>

      <div className="analytics2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>

        {/* Donut Chart */}
        <SpotlightCard style={{ padding: 22 }}>
          <div style={{ fontSize: 12, fontWeight: 800, color: c.text, marginBottom: 16, letterSpacing: "0.06em" }}>
            VIOLATION BREAKDOWN
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 18, flexWrap: "wrap" }}>
            <DonutChart data={violations} />
            <div style={{ display: "flex", flexDirection: "column", gap: 9, flex: 1, minWidth: 110 }}>
              {violations.map(d => (
                <div key={d.label} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ width: 8, height: 8, borderRadius: 2, background: d.color, flexShrink: 0 }} />
                  <div style={{ fontSize: 11, color: c.muted, flex: 1 }}>{d.label}</div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: c.text, fontFamily: "monospace" }}>{d.val}%</div>
                </div>
              ))}
            </div>
          </div>
        </SpotlightCard>

        {/* Bar Chart */}
        <SpotlightCard style={{ padding: 22 }}>
          <div style={{ fontSize: 12, fontWeight: 800, color: c.text, marginBottom: 8, letterSpacing: "0.06em" }}>
            MONTHLY TREND — {new Date().getFullYear()}
          </div>
          <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 14 }}>
            <AnimCounter
              value={ANALYTICS_DATA.monthly[ANALYTICS_DATA.monthly.length - 1]}
              color={c.accent}
              size={26}
            />
            <Chip label="+18.6% ↑" color={c.green} bg={c.greenDim} />
          </div>
          <BarChart data={ANALYTICS_DATA.monthly} months={ANALYTICS_DATA.months} />
        </SpotlightCard>
      </div>

      {/* ✅ Top Violation Zones — real location data */}
      <SpotlightCard style={{ padding: 22 }}>
        <div style={{ fontSize: 12, fontWeight: 800, color: c.text, marginBottom: 18, letterSpacing: "0.06em" }}>
          TOP VIOLATION ZONES
        </div>

        {topZones.length === 0 ? (
          <div style={{ color: c.muted, fontSize: 13, textAlign: "center", padding: "20px 0" }}>
            📍 No location data yet — submit a violation report with location to see zones here.
          </div>
        ) : (
          topZones.map((a, i) => (
            <div key={a.name} style={{ marginBottom: 13 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                <span style={{ fontSize: 13, color: c.text, fontWeight: 600 }}>
                  <span style={{ color: c.accent, fontFamily: "monospace", marginRight: 10 }}>
                    #{i + 1}
                  </span>
                  {a.name}
                </span>
                <span style={{ fontSize: 12, fontFamily: "monospace", color: c.muted }}>
                  {a.count} reports
                </span>
              </div>
              <div style={{ height: 7, background: "rgba(128,128,128,0.1)", borderRadius: 4, overflow: "hidden" }}>
                <div style={{
                  height: "100%", borderRadius: 4,
                  width: `${(a.count / maxZoneCount) * 100}%`,
                  background: `linear-gradient(90deg, ${c.accent}, ${c.purple})`,
                  transition: "width 1.4s cubic-bezier(0.23,1,0.32,1)",
                }} />
              </div>
            </div>
          ))
        )}
      </SpotlightCard>

      {/* Fine Summary */}
      <SpotlightCard style={{ padding: 22 }}>
        <div style={{ fontSize: 12, fontWeight: 800, color: c.text, marginBottom: 18, letterSpacing: "0.06em" }}>
          FINE SUMMARY
        </div>
        <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
          <div>
            <div style={{ color: c.muted, fontSize: 11, marginBottom: 4 }}>Total Fines Collected</div>
            <div style={{ color: c.accent, fontWeight: 800, fontSize: 26, fontFamily: "monospace" }}>
              ₹{stats.totalFine.toLocaleString("en-IN")}
            </div>
          </div>
          <div>
            <div style={{ color: c.muted, fontSize: 11, marginBottom: 4 }}>Average Fine</div>
            <div style={{ color: c.yellow, fontWeight: 800, fontSize: 26, fontFamily: "monospace" }}>
              ₹{stats.avgFine.toLocaleString("en-IN")}
            </div>
          </div>
          <div>
            <div style={{ color: c.muted, fontSize: 11, marginBottom: 4 }}>Fine Issued Cases</div>
            <div style={{ color: c.purple, fontWeight: 800, fontSize: 26, fontFamily: "monospace" }}>
              {stats.fineIssued}
            </div>
          </div>
        </div>
      </SpotlightCard>

    </div>
  );
}